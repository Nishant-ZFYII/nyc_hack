...
def add_safety_score(df):
    log.info("Step 4/5: Computing safety scores from NYPD complaints…")
    ...
    c_lat = nypd["latitude"].values
    c_lon = nypd["longitude"].values

    LAT_M = 111_320
    LON_M = 85_390
    RADIUS_M = 500

    # Convert radius to degrees for fast pre-filter
    dlat_deg = RADIUS_M / LAT_M
    dlon_deg = RADIUS_M / LON_M

    counts = []
    for lat, lon in tqdm(df[["latitude","longitude"]].values,
                         desc="Safety scoring", unit="resource", miniters=200):
    ...
    # Normalize: safety_score = 1 - (count / 99th percentile), clipped 0-1
...
def add_quality_score(df):
    log.info("Step 5/5: Computing quality scores from 311 complaints…")
    ...
    c_lat = c311["latitude"].values
    c_lon = c311["longitude"].values
    ...
    counts = []
    for lat, lon in tqdm(df[["latitude","longitude"]].values,
                         desc="Quality scoring", unit="resource", miniters=200):
        mask = (
            (c_lat >= lat - dlat_deg) & (c_lat <= lat + dlat_deg) &
            (c_lon >= lon - dlon_deg) & (c_lon <= lon + dlon_deg)
        )
