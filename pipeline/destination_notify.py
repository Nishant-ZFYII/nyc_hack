...
def confirm_destination_intent(case: dict, resource: dict,
    ...
    if isinstance(ec, str):
        ec = {"name": ec, "discord_webhook": ""}
    ec_webhook = cfg.get("ec_webhook", "") or ec.get("discord_webhook", "")
    if ec_webhook:
        payload = {"embeds": [_build_ec_embed(case, resource)]}
        if _post_webhook(payload, ec_webhook):
    ...
    if sla_minutes > 0 and (dest_webhook or ec_webhook):
    ...
    def _check():
        ...
        try:
            ...
            for intent in case.get("destination_intents", []):
                ...
                name = case.get("name", "User")
                webhook = cfg.get("dest_webhook", "") or cfg.get("ec_webhook", "")
                if not webhook:
                    return
                payload = {
                    "embeds": [{
